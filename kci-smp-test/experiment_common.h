...
/* op_type_t is defined in benchmark/dhrystone.h (included above). */
...
typedef struct dhrystone_context_t
{
  /* Workload configuration (set before dhrystone_init) */
  dhrystone_config_t config;

  /* Dhrystone workload data structures */
  ...
  /* Accumulated iteration count (updated by run_dhrystone_workload) */
  int throughput;
} dhrystone_context_t;
